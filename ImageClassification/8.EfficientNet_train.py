...
from ImageClassification.model import efficientnet_b0, efficientnet_b1, efficientnet_b2, efficientnet_b3, efficientnet_b4, efficientnet_b5, efficientnet_b6, efficientnet_b7 # 加载模型
...
def main(parameters_file_path):
    """ Main function to train the EfficientNet model on Flower5 dataset.
    Args: check in file: benchmark/config/EfficientNet_parameters.yaml
    """
    # 0. Load parameters
    parameters = load_model_parameters(parameters_file_path)
    epochs = parameters['epochs']
    batch_size = parameters['batch_size']
    input_size = parameters['input_size']['B0'] # input_size: {'B0': 224, 'B1': 240, 'B2': 260, 'B3': 300, 'B4': 380, 'B5': 456, 'B6': 528, 'B7': 600}
    ...
    print(f"Training EfficientNet for {epochs} epochs, batch size:{batch_size}, learning rate:{learning_rate}, num_classes:{num_classes}, saving to:{save_path}")
    ...
    # 2. Initialize model (efficientnet_b0, efficientnet_b1, efficientnet_b2, efficientnet_b3, efficientnet_b4, efficientnet_b5, efficientnet_b6, efficientnet_b7)
    net = efficientnet_b0(num_classes=num_classes).to(device) # Flower5 has 3 channels (RGB) and 5 classes

    # 加载预训练权重：
    load_pretrained = False
    if load_pretrained:
        model_weight_path = "./checkpoints/EfficientNet/EfficientNet_xxx.pth"
    ...
    if freeze_layers:
        for name, para in net.named_parameters():
            if ("features.top" not in name) and ("classifier" not in name): # 除最后的全连接层外，其他权重全部冻结
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        if val_accurate > best_acc:
            ...
            torch.save(net.state_dict(), f"{save_path}/Best_EfficientNet_epoch_{epoch + 1}.pth")
            print(f"Model saved at best accuracy: {best_acc:.3f}")

        # 每个epoch结束后，保存模型
        torch.save(net.state_dict(), f"{save_path}/EfficientNet_epoch_{epoch + 1}.pth")
...
if __name__ == '__main__':
    # 加载模型参数配置
    ALL_parameters_file_path = 'benchmark/config/EfficientNet_parameters.yaml'
