...
from ImageClassification.model import LeNet # 加载模型
...
def main(args):
    ...
    with torch.no_grad():
        ...
        net.eval()
        outputs = net(im)
        predict = torch.max(outputs if device == 'cpu' else outputs.cpu(), dim=1)[1].item()
    print(classes[int(predict.item())])


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--model_path', default='./checkpoints/LeNet/Best_LeNet_epoch_15.pth', type=str, help='path to the model')
