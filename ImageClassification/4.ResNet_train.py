...
from ImageClassification.model import ResNet # 加载模型
...
def main(parameters_file_path):
    """ Main function to train the ResNet model on Flower5 dataset.
    Args: check in file: benchmark/config/ResNet_parameters.yaml
    """
    # 0. Load parameters
    ...
    print(f"Training ResNet for {epochs} epochs, batch size:{batch_size}, learning rate:{learning_rate}, num_classes:{num_classes}, saving to:{save_path}")
    ...
    # 2. Initialize model (ResNet)
    net = ResNet(num_classes=num_classes).to(device) # Flower5 has 3 channels (RGB) and 5 classes
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        if val_accurate > best_acc:
            ...
            torch.save(net.state_dict(), f"{save_path}/Best_ResNet_epoch_{epoch + 1}.pth")
            print(f"Model saved at best accuracy: {best_acc:.3f}")

        # 每个epoch结束后，保存模型
        torch.save(net.state_dict(), f"{save_path}/ResNet_epoch_{epoch + 1}.pth")
...
if __name__ == '__main__':
    # 加载模型参数配置
    ALL_parameters_file_path = 'benchmark/config/ResNet_parameters.yaml'
